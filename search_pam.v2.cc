...
int main(int argc, char* argv[])
{
	...
	opt_n["in_fasta_file"]   = 1;
	opt_sl["i"] = "in_fasta_file";

	opt_n["is_pam_head"] = 1;

	opt_n["target_length"] = 1;
    ...
    string conf_file;
    string log_file;
	string in_fasta_file;
	int target_length = 23;
	short is_pam_head = 0;
	string pam = "NTT";
	// }}}
	///
	...
	for (map<string,string>::iterator it=opt.begin(); it!=opt.end(); it++) {
		...
		if (name.compare("out_prefix")==0 ) {
			...
			log_file = value;
		} else if ( name=="in_fasta_file" ) {
			in_fasta_file = value;
		} else if ( name=="is_pam_head" ) {
